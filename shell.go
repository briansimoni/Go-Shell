...
func invokeProgram(programName string, args []string) error {
	...
	// determine where the output of the program should go
	var out *os.File
	if (len(args) > 0) && (args[len(args)-1] == "&") {
		fmt.Println("should print to background")

		args = args[:len(args)-1]

		//out = os.NewFile(uintptr(1), "/dev/null")
		out = nil
	} else {
		out = os.Stdout
	}
	...
	attribs.Files = []*os.File{nil, out, os.Stderr}
